...
### Vorgeschlagene Verbesserungen
> NOTE 2026-06-11: First attempt edited `~/.claude/skills/*` directly — those are sync targets (unpacked `.skill` bundles) and were overwritten. All four items were then applied in the SOURCE repo `~/Local Sites/claude-skills` (commit `bda7e6e`, pushed to main), plus a guard added to both learning-backlog phases so future learning edits target the source repo directly.
- [x] `guidelines/security.md`: Add rule under Section XI — prompt template files (`src/prompts/*.md`) must be audited for untrusted placeholder isolation on every full-audit pass, not only the JS callers. Any `{{placeholder}}` whose value originates from external data (GSC, SerpAPI, LLM output) must be preceded by a `<<<UNTRUSTED_*_START>>>` block in the template itself. (done: claude-skills bda7e6e, security.md XII + 2-security.md)
- [x] Audit orchestrator prompt (or `agents/fix-agent.md`): Add instruction — when a new shared utility is extracted (new `lib/*.js`), the fix-agent must grep all files in `src/` for the pattern being centralised and migrate every occurrence, not only the files named in the finding. Explicitly list the grep command to run. (done: claude-skills bda7e6e, fix-agent.md Sonderfall + both SKILL.md orchestrators)
- [x] Audit orchestrator pre-flight: Add a hard check — if no `vitest`, `jest`, `node:test`, or `mocha` is configured in `package.json` after 3 consecutive full-audits, escalate to a Critical finding in the next audit rather than a gap note. This unblocks automated regression verification for all future fix-agents. (done: claude-skills bda7e6e, full-audit Phase 0.4 + 3c; seo-cli itself now has vitest, so the check resolves green here)
- [x] Audit scope definition: Explicitly add `src/prompts/*.md` as a Security-dimension audit target (alongside `.js` files) so template files are never skipped in the initial file-selection pass. (done: claude-skills bda7e6e, scope-context-batching.md)
